  ...
  //  public String serverip="192.168.0.52";
   ...
   // public int port=2000;  //服务器端口
    ...
    public IBinder onBind(Intent intent) {
        ...
        binder=new ibinder(){
            @Override
            public boolean sendOrder(final String order,final int start_crc){   //重写发送命令的方法
                ...
                new Thread()   //新建子线程，发送命令
                {
                    public void run(){
                        ...
                        try
                        {
                            ...
                            byte[] bt=null;
                            bt=Converts.HexString2Bytes(toServer);
                            String str=toServer+Converts.GetCRC(bt, start_crc, bt.length)+"0d0a";   //添加校验码和包尾
                            ...
                            try
                            {
                                ...
                                bt = null;
                                bt = Converts.HexString2Bytes(toServer);
                                str = toServer + Converts.GetCRC(bt, start_crc, bt.length) + "0d0a";   //添加校验码，和包尾
...
//                                    handler1.sendMessage(msg);
                                    ...
                                    intent.putExtra("ContentNum",count);
                                    ...
                                    s = s.replace(" ", ""); //去掉空格
                                    Log.i("Order", "收到数据："+s);
                                   // SwitchState.setState(content,count);
                                }
                            }
                        }
                    }
    ...
    public class ibinder extends Binder {
         ...
         * @param start_crc 校验开始的位数，第六感与开关不同，开关是2，第六感是4.
         * @return  发送成功返回true，失败返回false
         */
        public boolean sendOrder(String order,int start_crc){
            return true;
        }
    }
